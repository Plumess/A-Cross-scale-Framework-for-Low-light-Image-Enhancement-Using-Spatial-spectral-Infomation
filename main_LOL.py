...
import time
import argparse
from model_LOL import EnhanceNet
from dataset_LOL import myDataset
...
parser.add_argument('--epoch', dest='epoch', type=int, default=1000, help='number of total epochs')
...
parser.add_argument('--dir', dest='dir', default='ours_LOL', help='directory')
parser.add_argument('--test_dir', dest='test_dir', default=r'D:/LHY/LLE/Raw_Datasets/LOL/eval15/low',
                    help='directory for testing inputs')  # 在dataset_LOL用绝对路径指定
parser.add_argument('--train_dir', dest='train_dir', default=r'',
                    help='directory for training inputs')  # 路径后面有斜杆
parser.add_argument('--continue_train', dest='continue_train', default=200)
...
def train():
    ...
    if args.continue_train:
        checkpoint = torch.load("D:/LHY/LLE/checkpoint_ours_lol/ 200_state_final.pth")
...
def test():
    checkpoint = torch.load("D:/LHY/LLE/checkpoint_ours_lol/ 200_state_final.pth")
    enhancenet.load_state_dict(checkpoint['enhance'], strict=False)
    print('load weights successfully')
    args.batch_size = 1
    test_set = myDataset(args.test_dir, 'test', args.patch_size)
    ...
    with torch.no_grad():
        for (step, data) in enumerate(dataLoader):
            ...
            save_images(os.path.join('test_results_' + args.dir, '%d.png' % step), out_cpu.detach().numpy())
            save_images(os.path.join('test_results_'+args.dir, '%d_gt.png'%(step)), high_im.detach().numpy())
