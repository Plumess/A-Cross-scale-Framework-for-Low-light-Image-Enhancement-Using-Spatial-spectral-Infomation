...
class seblock(nn.Module):
    def __init__(self, in_channel=32, out_channel=32):
        ...
        self.fc = nn.Sequential(nn.Linear(in_channel, out_channel), nn.ReLU(), nn.Linear(out_channel, out_channel),
                                nn.Sigmoid())
...
class single_block1(nn.Module):  # Channel Attention Block + Conv2d
    def __init__(self, in_channel=32, out_channel=32):
        super(single_block1, self).__init__()
        self.conv1 = nn.Sequential(seblock(in_channel, in_channel), nn.Conv2d(in_channel, out_channel, 3, padding=1),
                                   nn.LeakyReLU(0.2))
        self.conv2 = nn.Sequential(nn.Conv2d(out_channel, out_channel, 3, padding=1), nn.LeakyReLU(0.2))
...
class EnhanceNet(nn.Module):  # final
    def __init__(self, channel=64):
        ...
        self.ffc6 = nn.Sequential(FFCResnetBlock(128), ResLeWinTransformerLayer(128, 128, 256, 2),
                                  single_block(128, 64))
        self.ffc7 = nn.Sequential(FFCResnetBlock(256), ResLeWinTransformerLayer(256, 256, 128, 4), FFCResnetBlock(256),
                                  ResLeWinTransformerLayer(256, 256, 128, 4))
        ...
        self.fusion_conv = nn.Sequential(nn.Conv2d(128, 64, 1, 1, 0), single_block1(64, 64), nn.Conv2d(64, 32, 1, 1, 0),
                                         single_block1(32, 32))
